...
interface BusinessDataContextType {
  ...
  owners: Owner[];
  pets: Pet[];
  appointments: (Appointment & { petName: string; ownerName: string; ownerPhone: string })[];
  medicalRecords: any[];
...
export const BusinessDataProvider: React.FC<BusinessDataProviderProps> = ({ children }) => {
  ...
  const isLoading = isLoadingOwners || isLoadingAppointments || isLoadingMedical;
  ...
  // Actions
  const addOwner = async (ownerData: Omit<Owner, 'id' | 'createdAt' | 'updatedAt'>) => {
    setIsLoadingOwners(true);
    try {
      // Simulate API call
  ...
  const getStats = () => {
    ...
    return {
      ...
      completedAppointmentsThisMonth: appointments.filter(apt => 
